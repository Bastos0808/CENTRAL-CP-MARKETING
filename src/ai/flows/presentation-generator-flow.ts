    ...
    const llmResponse = await ai.generate({
        ...
        prompt: `
            ...
            **Dados do Diagnóstico (R1):**
            ...
            - **Faturamento Médio Atual:** ${input.faturamentoMedio}
            - **Meta de Faturamento (Próximos 6 meses):** ${input.metaFaturamento}
            - **Ticket Médio:** ${input.ticketMedio}
            - **Origem dos Clientes Hoje:** ${input.origemClientes}
            - **Tempo de Empresa:** ${input.tempoEmpresa}
            - **Motivação para Investir em Marketing:** ${input.motivacaoMarketing}
            - **Experiência Anterior com Marketing:** ${input.investimentoAnterior}
            - **Tentativas Anteriores que Falharam:** ${input.tentativasAnteriores}
            - **Principal Gargalo Identificado:** ${input.principalGargalo}
            - **Custo Mensal Estimado do Problema:** ${input.custoProblema}
            - **Envolvidos na Decisão:** ${input.envolvidosDecisao}
            - **Orçamento Previsto/Disponibilidade de Investimento:** ${input.orcamentoPrevisto}
            - **Prazo para Decisão:** ${input.prazoDecisao}
            ...
            **Estrutura da Apresentação (Slides)**
            ...
            6.  **whyCpSlide:**
                ...
                -   **Conteúdo (em 3 bullet points, conectando os diferenciais à dor do cliente):**
                    -   "**Mentoria e Agilidade:** Para garantir alinhamento e agilidade, entregamos o projeto estratégico em 10 dias com uma mentoria de apresentação, permitindo que a execução comece rapidamente para atacar o gargalo de ${input.principalGargalo}."
                    ...
                    -   "**Foco em Performance de Negócio:** Enquanto outros se concentram em métricas de vaidade, nossa obsessão é o crescimento do seu faturamento. Cada ação é pensada para aproximá-lo da sua meta de ${input.metaFaturamento}."
            ...
            8.  **investmentSlide:**
                ...
                -   **Itens:** Gere uma descrição detalhada e atraente dos itens incluídos no plano, baseada nos pacotes selecionados. Destaque os principais benefícios e como eles se conectam para formar uma solução completa.
